...
// Y Resolution	
	...
	if (rc < 0)
		return rc;
	minor = ((buf_recv[2])) | ((buf_recv[3] & 0xf0) << 4);
	ts->y_resolution =minor*480/1152;
...
static int elan_ktf2k_ts_setup(struct i2c_client *client)
{
	...
	rc = __hello_packet_handler(client);            //fail 0x80  sucess 0x00
...
//Check for FW_VERSION, if 0x0000 means FW update fail!
	...
	return rc;                                  //fail 0x80, sucess 0x00
}
...
static void elan_ktf2k_ts_report_data(struct i2c_client *client, uint8_t *buf)
{
    ...
    switch (buf[0]) {
    	...
    	case TEN_FINGERS_PKT:
			if (num == 0) {
				//CML: HOME is 0x04 and it's needed for Firefox OS
				if (buf[BUTTON_ID_INDEX] == 0x04) {
                    button_state=0x04;
...
#if IAP_PORTION
	//cellon,Zepeng,modify start,2013-01-05,for TP fw_updata	      
